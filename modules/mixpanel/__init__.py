...
x = urllib.urlencode({'data':"dgdsgfshgsafhash",
                        'verbose': 1,
                        'ip': 0,
                        'api-key': "gggggg"}).encode('utf-8')
...
class Consumer(object):
    ...
    def _write_request(self, request_url, json_message, api_key=None):
        ...
        encoded_data = urllib.urlencode(data).encode('utf8') #----> was: urllib.parse.urlencode(data).encode('utf8')
        try:
            request = urllib2.Request(request_url, encoded_data)#----> was: urllib.request.Request(request_url, encoded_data)
            ...
            if self._request_timeout is not None:
                response = urllib2.urlopen(request, timeout=self._request_timeout).read()#---> was: urllib.request.urlopen(request, timeout=self._request_timeout).read()
            else:
                response = urllib2.urlopen(request).read()  #----> urllib.request.urlopen(request).read()
        except:
            return
